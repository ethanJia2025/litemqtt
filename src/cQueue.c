...
{
    cQueue_t *pq = NULL;
    pq = (cQueue_t *)cqueue_malloc(sizeof(cQueue_t));
    if(pq != NULL){             /*      Initialize the queue    */
        pq->QMemory = (void **)cqueue_malloc(size*sizeof(void *));
...
/*
 * Description  :   delete the queue, and cqueue_free the stroage area
 *
